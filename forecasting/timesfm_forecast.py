...
def main(output_dir: Path, batch_size: int) -> None:
    ...
    for i in tqdm(range(0, len(tickers), batch_size), desc="TimesFM"):
        ...
        for j, ticker in enumerate(batch_tickers):
            ...
            for col, bdays in HORIZON_MAP:
                idx = bdays - 1
                if idx < len(yhat):
                    # yhat[idx] may be a small array depending on timesfm version.
                    val = np.asarray(yhat[idx]).reshape(-1)[0]
                    result[col] = round(max(float(val), 0.0), 4)
                else:
    ...
    out_path = output_dir / f"timesfm_osl_{today}_single_run.tsv"
